...
const ReviewsPage = () => {
    ...
    return (
        <div className="min-h-screen bg-light-bg transition-colors pb-10">
            <Navbar />
            ...
            <main className="max-w-3xl mx-auto px-4 py-8 md:py-12">
                ...
                <div className="flex items-center space-x-4 mb-8">
                    <button
                        onClick={() => navigate(-1)}
                        className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-gray-800 shadow-sm border border-gray-100 hover:bg-brand-primary/10 transition-colors"
                    >
                        <ChevronLeft size={20} />
                    </button>
                    <div>
                        <h1 className="text-xl md:text-2xl font-heading font-extrabold text-gray-800 uppercase tracking-tight">Customer Reviews</h1>
                ...
                <Link to={`/product/${id}`} className="block bg-white rounded-[32px] p-6 mb-10 border border-gray-100 shadow-sm hover:shadow-md transition-all group">
                    <div className="flex items-center space-x-6">
                        <div className="w-20 h-20 rounded-2xl bg-gray-50 p-3 overflow-hidden border border-gray-100 shrink-0">
                            <img
                        ...
                        <div className="flex-1 min-w-0">
                            <h2 className="text-lg font-black text-gray-900 truncate mb-2">{product.name}</h2>
                            <div className="flex items-center space-x-4">
                                <div className="flex items-center px-3 py-1 bg-yellow-50 rounded-lg text-yellow-600 space-x-1.5">
                                                ...
                                                <div className="flex items-center">
                                                    {[1, 2, 3, 4, 5].map((s) => (
                                                        <Star
                                                            key={s}
                                                            size={10}
                                                            className={`${s <= rev.rating ? 'fill-yellow-500 text-yellow-500' : 'text-gray-200 '}`}
                                                        />
                                                    ))}
                                                </div>
                                                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50 px-2 py-0.5 rounded">Verified Buyer</span>
                                            </div>
                                        </div>
                                    </div>
